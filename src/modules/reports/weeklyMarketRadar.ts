...
export async function generateWeeklyMarketRadar(): Promise<string> {
  const db = await CareerDatabase.open();
  const top = db.query<{ career_track: string; total: number }>("SELECT career_track, COUNT(*) as total FROM jobs GROUP BY career_track ORDER BY total DESC");
