...
export function buildInformalMessage(kind: "detalhes" | "negociar" | "recusar" | "confirmar" | "cobrar", opportunity?: InformalOpportunity): string {
  if (kind === "negociar") {
  ...
  if (kind === "recusar") return "Obrigado pelo convite e pela lembrança. Para essa data/condição não consigo confirmar, mas fico à disposição para próximas oportunidades.";
  if (kind === "confirmar") return "Obrigado. Confirmo meu interesse, condicionado aos dados de local, horário, função, valor e forma de pagamento combinados por escrito.";
  if (kind === "cobrar") return "Olá, tudo bem? Passando para confirmar a previsão de pagamento referente ao trabalho realizado. Fico no aguardo, obrigado.";
